//! Anthropic API key authentication management.
//!
...
/// Keyring service name for Anthropic credentials.
const KEYRING_SERVICE: &str = "xzardgz-anthropic";

/// Keyring entry key for the Anthropic API key.
...
/// Manages Anthropic API key storage and status queries.
///
...
impl AnthropicAuth {
    /// Creates an `AnthropicAuth` using the given environment variable name.
    ///
    /// # Arguments
    ///
    /// * `api_key_env` - The env var name that holds the API key,
    ///   typically `"ANTHROPIC_API_KEY"`.
    ///
    /// # Examples
    ///
    /// ```
    /// use xzardgz::auth::AnthropicAuth;
    ///
    /// let auth = AnthropicAuth::new("ANTHROPIC_API_KEY");
    /// ```
...
mod tests {
    ...
    /// Returns an `AnthropicAuth` bound to a test-specific env var name.
    fn test_auth(env_var: &str) -> AnthropicAuth {
        AnthropicAuth::new(env_var)
    }
    ...
    #[test]
    fn test_anthropic_auth_status_returns_not_authenticated_when_no_credentials() {
        // Use a unique placeholder env var that will never be set in practice.
        // Assumes the keyring service "xzardgz-anthropic" has no stored key
        // in the current environment (guaranteed in CI).
        let auth = test_auth("XZARDGZ_TEST_ANTHROPIC_NOT_SET_PLACEHOLDER");
    ...
    #[test]
    fn test_anthropic_auth_status_returns_credential_present_when_env_set() {
        temp_env::with_var(
            "XZARDGZ_TEST_ANTHROPIC_ENV_KEY_1",
            Some("sk-ant-test-value"),
            || {
                let auth = test_auth("XZARDGZ_TEST_ANTHROPIC_ENV_KEY_1");
                let status = auth.status();
                assert!(
    ...
    #[test]
    fn test_anthropic_auth_get_key_returns_env_var_value() {
        temp_env::with_var(
            "XZARDGZ_TEST_ANTHROPIC_ENV_KEY_2",
            Some("sk-ant-test-secret"),
            || {
                let auth = test_auth("XZARDGZ_TEST_ANTHROPIC_ENV_KEY_2");
                let key = auth.get_key();
                assert_eq!(
                    key,
                    Some("sk-ant-test-secret".to_string()),
    ...
    #[test]
    fn test_anthropic_auth_get_key_returns_none_when_no_env_var() {
        // Use a unique placeholder guaranteed not to be set.
        // Assumes keyring has no pre-existing key (guaranteed in CI).
        let auth = test_auth("XZARDGZ_TEST_ANTHROPIC_NONE_PLACEHOLDER");
        let key = auth.get_key();
        assert!(
            key.is_none(),
    ...
    // ------------------------------------------------------------------
    // remove_key (smoke test)
    // ------------------------------------------------------------------

    #[test]
    fn test_anthropic_auth_remove_key_succeeds_when_no_entry() {
        // delete_credential on a missing entry should succeed silently.
        let auth = test_auth("XZARDGZ_TEST_ANTHROPIC_DUMMY");
