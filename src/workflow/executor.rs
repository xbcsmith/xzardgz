...
pub enum ExecutionInput {
    ...
    ScanOnly {
        ...
        /// Optional tracing identifier for this run.
        ///
        /// When `None`, a fresh ULID is generated by the executor.
...
pub struct ExecutionResult {
    ...
    ///
    /// Threaded from the inbound watcher task (for watcher-triggered runs) or
    /// freshly generated as a ULID for CLI-triggered runs. Survives `--resume`
    /// because it is persisted on `WorkspaceState`.
...
impl WorkflowExecutor {
    ...
    async fn run_plan(&self, plan: WorkflowPlan) -> Result<ExecutionResult> {
        ...
        // Stage 4: open or create workspace, resolving the correlation_id.
        //
        // Priority: (1) plan.correlation_id if supplied by the caller; (2) for
        // resume, the id persisted on the existing workspace; (3) a freshly
        // generated ULID.
        let supplied_cid = plan.correlation_id.clone();
        ...
        // Backfill correlation_id for pre-Phase-3 state files opened via resume.
        if workspace.state.correlation_id.is_empty() {
            workspace.state.correlation_id =
                supplied_cid.unwrap_or_else(|| Ulid::new().to_string());
            workspace.save()?;
        }
        let correlation_id = workspace.state.correlation_id.clone();
    ...
    async fn run_watcher_task(&self, task: WatcherTaskMessage) -> Result<ExecutionResult> {
        let started_at = Utc::now();
        let mut plan = self.watcher_task_to_plan(&task);
        // Carry the task's correlation_id into the plan so run_plan persists it.
        plan.correlation_id = Some(task.correlation_id.clone());
    ...
    async fn run_scan_only(
        ...
        // Backfill correlation_id for pre-Phase-3 state files.
        if workspace.state.correlation_id.is_empty() {
            workspace.state.correlation_id =
                correlation_id.unwrap_or_else(|| Ulid::new().to_string());
            workspace.save()?;
        }
        let run_correlation_id = workspace.state.correlation_id.clone();
        let workspace_id = workspace.id().to_string();
    ...
    async fn run_plugin_only(
        ...
        // Backfill correlation_id for pre-Phase-3 state files or fresh
        // workspaces where the caller did not supply one.
        if workspace.state.correlation_id.is_empty() {
            workspace.state.correlation_id = correlation_id
                .clone()
    ...
    fn execute_dry_run(
        ...
        Ok(ExecutionResult {
            ...
            completed_at: Utc::now(),
            is_dry_run: true,
            correlation_id: workspace.state.correlation_id.clone(),
        })
    }
    ...
    fn write_step_reports(
        ...
        envelope.correlation_id = Some(workspace.state.correlation_id.clone());
...
mod tests {
    ...
    // ------------------------------------------------------------------
    // Phase 3: correlation_id propagation
    // ------------------------------------------------------------------

    #[tokio::test]
    async fn test_execute_local_plan_produces_non_empty_correlation_id() {
        // A CLI-triggered run must produce a workspace with a non-empty,
        // stable correlation_id even when none is supplied by the caller.
        let repo_dir = TempDir::new().unwrap();
        let ws_dir = TempDir::new().unwrap();
        let executor = make_executor_with_success_plugin(ws_dir.path().to_str().unwrap());
        let mut plan = make_test_plan(
        ...
        plan.correlation_id = None; // simulate no caller-supplied id

        let result = executor
            .execute(ExecutionInput::LocalPlan(Box::new(plan)))
            .await
            // SAFETY: test repo and workspace are freshly created.
            .unwrap();

        assert!(result.success);
        assert!(
            !result.correlation_id.is_empty(),
            "correlation_id must be non-empty"
        );
    }

    #[tokio::test]
    async fn test_execute_local_plan_resume_preserves_original_correlation_id() {
        // When a run is resumed, the correlation_id from the original workspace
        // state must be used rather than generating a new one.
        let repo_dir = TempDir::new().unwrap();
        let ws_dir = TempDir::new().unwrap();
        let executor = make_executor_with_success_plugin(ws_dir.path().to_str().unwrap());

        // First run: supply an explicit correlation_id.
        let expected_cid = "test-correlation-id-stable-01ARZ3NDEKTSabcdefgh".to_string();
        let mut plan = make_test_plan(
            repo_dir.path().to_str().unwrap(),
            ws_dir.path().to_str().unwrap(),
        );
        plan.correlation_id = Some(expected_cid.clone());
        ...
        let first = executor
            ...
            // SAFETY: test repo and workspace are freshly created.
            .unwrap();
        assert_eq!(first.correlation_id, expected_cid);

        // Second run: resume the same workspace WITHOUT supplying correlation_id.
        let mut plan2 = make_test_plan(
            repo_dir.path().to_str().unwrap(),
            ws_dir.path().to_str().unwrap(),
        );
        plan2.resume = true;
        plan2.correlation_id = None;

        let second = executor
            .execute(ExecutionInput::LocalPlan(Box::new(plan2)))
            .await
            // SAFETY: workspace from first run exists on disk.
            .unwrap();

        assert_eq!(
            second.correlation_id, expected_cid,
            "--resume must preserve the original correlation_id"
        );
    }
}
